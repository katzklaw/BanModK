        ...
        {
            {
                ...
                if (AmongUsClient.Instance == null || AmongUsClient.Instance.IsGameStarted || Utils.IsInGame || __instance.startState == GameStartManager.StartingStates.Starting) return;
    ...
    public class GameStartManagerUpdatePatch
    {
        ...
        public static bool Prefix(GameStartManager __instance)
        {
            try
            {
                ...
                if (AmongUsClient.Instance.IsGameStarted || Utils.IsInGame || __instance == null || __instance.startState == GameStartManager.StartingStates.Starting) return false;
                ...
                if (BanMod.AutoStart != null && BanMod.AutoStart.Value)
                {
                    BanMod.UpdateTime++;
                    if (BanMod.UpdateTime >= 50)
                    {
                        BanMod.UpdateTime = 0;
                        if (((GameData.Instance?.PlayerCount >= MinPlayer && Timer <= MinWait) || Timer <= MaxWait) && !Utils.IsCountDown)
                        {
        ...
        {
            {
                if (AmongUsClient.Instance == null || AmongUsClient.Instance.IsGameStarted || Utils.IsInGame || __instance == null || __instance.startState == GameStartManager.StartingStates.Starting) return;
